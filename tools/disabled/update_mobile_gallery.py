...
files currently present in `assets/wallpapers/mobile/`.

Usage:
  python tools/update_mobile_gallery.py
...
def list_images(dirpath: Path):
    ...
    files = [p.name for p in sorted(dirpath.iterdir()) if p.is_file() and p.suffix.lower() in exts]
    return files
...
def main():
    ...
    print(f'Updated mobile.html with {len(images)} images (v={version})')
